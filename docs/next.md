...
- Fixed EC2 UI API launches that failed before monitor progress appeared when the default seed query run was missing on the server: the default seed `api_queries.csv` now lives under tracked `config/seed_queries/...`, launch/resume preflight rejects missing seed files before starting a process, and the UI surfaces the exact missing path instead of showing a misleading launched state.
...
- Added first teammate email `junhao59@163.com` to the Cloudflare Access policy for `GEO Admin Console`.
