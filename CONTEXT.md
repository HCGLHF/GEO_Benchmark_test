...
- **Industry isolation**: every cloud corpus, artifact, query set, and benchmark run belongs to an explicit `industry_id` so unrelated industry datasets do not mix.
...
- Use `powershell -ExecutionPolicy Bypass -File .\scripts\run_full_api_parallel_with_watch.ps1 -RunMode test` for a minimal API-chain check, roughly 2 seeded queries and 4 seeded API calls per model.
